...
# <a id="iOS SDK Setup"></a>iOS (Objective C) SDK Setup
...
>**Important:** For any Header file that uses the Optimove SDK, add an import with your module name followed by '-Swift.h', such as *#import "HelloWorld-Swift.h"*
...
The initialization must be called **as soon as possible**, unless the tenant has its own Firebase SDK. In this case, start the initialization right after calling `FirApp configure`.
...
To track which screens the user has visited in your app, send a *setScreenEvent* message to the shared Optimove instance:
  ...
  ````swift
[Optimove.sharedInstance setScreenEventWithViewControllersIdentifiers:url]
````
...
````swift
CustomEvent* event = [[CustomEvent alloc] initWithParam:customInput];
